 ...
 *
 * cont_args is a NULL-terminated array of strings, i.e., the last element of
...
/*** STUDENT CODE BELOW (q5, q6, q7, q8, q9) ***/
 ...
 */
int contain(struct container cont, char* cont_args[]) {
    ...
    print_container_cmd(cont_args);


    // Make the filesystem for the container.
    if (contfs_make(&cont.fs, cont.image) < 0)
        errx(EXIT_FAILURE, "failed making filesystem for container %s",
                cont.hostname);
    ...
    if (cgroup_make(&cont.cgroup, cont.memory_MB, cont.cpu_perc) < 0) {
        ...
        errx(EXIT_FAILURE, "failed making cgroup for container %s",
                cont.hostname);
    }
    ...
    if ((cont_pid = clone_to_container(&cont)) > 0) {
        ...
        if (finalize_host(&cont) < 0) {
            ...
            if (contfs_demake(&cont.fs) < 0)
                warn("failed demaking filesystem for container %s",
                        cont.hostname);

            errx(EXIT_FAILURE,
                    "failed setting up container environment from host");
        }
 ...
 *
 * Parse the arguments to build a struct container, and treat the container's
 * arguments.
...
int main(int argc, char* argv[]) {
    ...
    if (argc < FIRST_CONTAINER_ARG) {
        fprintf(stderr,
                "Usage: %s HOSTNAME IMAGE MEMORY_MB CPU_PERC IP_ADDRESS "
                "[ARGS...]\n", argv[0]);
        return -2;
    }
    ...
    memcpy(cont_args, &argv[FIRST_CONTAINER_ARG],
            nb_cont_args * sizeof(argv[0]));
    cont_args[nb_cont_args] = NULL;

    ret = contain(cont, cont_args);
